...
import argparse
import numpy as np
from utils.mydata_loader import DataLoader
import glob
from tqdm import trange
from net.mysgcn_model import SparseGCNModel
...
parser.add_argument('--file_path', default='CVRP_train', help='')
parser.add_argument('--eval_file_path', default='CVRP_val', help='')
...
while epoch < args.n_epoch:
    ...
    for batch in trange(30 * 40):
        node_feat, edge_feat, alpha_feat, label, edge_index, inverse_edge_index = dataLoader.next_batch()
        ...
        y_edges, loss_edges, y_nodes = net.forward(node_feat, edge_feat, alpha_feat, edge_index, inverse_edge_index, label, edge_cw, n_edges)
    ...
    if epoch % args.eval_interval == 0:
        ...
        for n_node in [100, 200, 500]:
            ...
            for eval_batch in trange(1000 // args.eval_batch_size):
                ...
                alpha_feat = dataset["alpha_feat"][eval_batch * args.eval_batch_size:(eval_batch + 1) * args.eval_batch_size]
                ...
                with torch.no_grad():
                    ...
                    alpha_feat = Variable(torch.FloatTensor(alpha_feat).type(torch.cuda.FloatTensor), requires_grad=False).view(args.eval_batch_size, -1, 1) # B x 1000 x 2
