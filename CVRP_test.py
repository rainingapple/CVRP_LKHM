...
def write_candidate(dataset_name, instance_name, candidate, n_nodes_extend):
    ...
    with open("result/" + dataset_name + "/candidate/" + instance_name + ".txt", "w") as f:
        ...
        for j in range(n_nodes_extend):
            ...
            bad_node = -99
            for _ in range(20):
                node_id = int(candidate[j, _]) + 1
                if node_id not in exist_flag:
                    exist_flag.append(node_id)
                else:
                    bad_node = node_id
                    break
            for _ in range(20):
                ...
                node_id = int(candidate[j, _]) + 1
                if node_id != bad_node:
                    s_cand += " " + str(node_id) + " " + str(cand_count * 100)
                    cand_count = cand_count + 1
            s_cand = str(j + 1) + " 0 " + str(cand_count) + s_cand
            f.write(s_cand + "\n")
...
def generate_feat(dataset_name, instance, instance_name, max_nodes):
    ...
    with open(feat_filename, "w") as f:
        check_call(["./fgcand20",para_filename], stdout=f)
    ...
    x = np.array(instance[0])
    tkk = len(x) - 1 
    tmpadd = int(tkk * 1.15) - len(x)
    x = np.concatenate([x] + [x[0:1, :] for _ in range(tmpadd)], 0)
    n_nodes = len(x)
    n_neighbours = 20
    dist = x.reshape(n_nodes, 1, 2) - x.reshape(1, n_nodes, 2)
    dist = np.sqrt((dist ** 2).sum(-1)) # n_nodes * n_nodes

    # create empty ndarray(n_samples, n_nodes) for index
    edge_index = np.zeros((1, n_nodes,n_neighbours),dtype=int) - 1
    alpha_feat = np.zeros((1, n_nodes,n_neighbours),dtype=float) - 1

    # add candidate edge in the opt-1-tree
    for i in range(0,len(cand)):
        per_cand = cand[i].split()
        n_from = int(per_cand[0]) - 1
        for j in range(1,min((len(per_cand)+1)//2 , n_neighbours + 1)):
            n_to = int(per_cand[2*j-1]) - 1
            if n_to not in edge_index[0][n_from]:
                edge_index[0][n_from][j-1] = n_to
                if float(per_cand[2*j]) == 0:
                    alpha = 0
                else:
                    alpha = max(0,np.log10(float(per_cand[2*j]))+5)
                alpha_feat[0][n_from][j-1] = alpha

    for idx in range(n_nodes):
        dist_single = np.argmax(dist[idx])
        pos = np.where(edge_index[0][idx] == -1)[0]
        edge_index[0][idx][pos] = dist_single
        alpha_feat[0][idx][pos] = 6

    return edge_index, alpha_feat, 0, len(cand)

def infer_SGN(net, dataset_node_feat, dataset_edge_index, dataset_edge_feat, dataset_alpha_feat, dataset_inverse_edge_index, batch_size=100):
    candidate = []
    for i in trange(dataset_edge_index.shape[0] // batch_size):
        ...
        alpha_feat = dataset_alpha_feat[i * batch_size:(i + 1) * batch_size]
        ...
        alpha_feat = Variable(torch.FloatTensor(alpha_feat).type(torch.cuda.FloatTensor), requires_grad=False).view(batch_size, -1, 1)
        ...
        y_edges, _, y_nodes = net.forward(node_feat, edge_feat, alpha_feat, edge_index, inverse_edge_index, None, None, 20)
        ...
        candidate.append(candidate_index[:, :, :20])
...
def eval_dataset(dataset_filename, method, args, rerun=True, max_trials=1000):
    ...
    if method == "NeuroLKH":
        ...
        edge_index, alpha_feat, feat_runtime, n_nodes_extend = list(zip(*feats))
        feat_runtime = np.sum(feat_runtime)
        feat_start_time = time.time()
        edge_index = np.concatenate(edge_index, 0)
        alpha_feat = np.concatenate(alpha_feat, 0)
        ...
        with torch.no_grad():
            candidate = infer_SGN(net, node_feat, edge_index, edge_feat, alpha_feat, inverse_edge_index, batch_size=args.batch_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default='saved/exp1/70.pt', help='')
    parser.add_argument('--n_samples', type=int, default=1000, help='')
    parser.add_argument('--batch_size', type=int, default=100, help='')
    parser.add_argument('--lkh_trials', type=int, default=100, help='')
    parser.add_argument('--neurolkh_trials', type=int, default=100, help='')
    ...
    lkh_objs, lkh_penalties, lkh_runtimes, _, _ = eval_dataset(args.dataset, "LKH", args=args, rerun=False, max_trials=args.lkh_trials)
