...
class SparseGCNModel(nn.Module):
    ...
    def forward(self, x_nodes, x_edges, alpha_edges, edge_index, inverse_edge_index, y_edges, edge_cw, n_edges):
        batch_size, num_nodes, _ = x_nodes.size()
        x = self.nodes_embedding(x_nodes)  # batch_size x n_node x hidden_dimension
        e = self.edges_embedding(torch.cat((x_edges,alpha_edges),dim=2))  # batch_size x n_node * n_edge x hidden_dimension
    ...
    def forward_finetune(self, x_nodes, x_edges, alpha_edges, edge_index, inverse_edge_index, n_edges):
        with torch.no_grad():
            batch_size, num_nodes, _ = x_nodes.size()
            x = self.nodes_embedding(x_nodes)
            e = self.edges_embedding(torch.cat((x_edges,alpha_edges),dim=2))
