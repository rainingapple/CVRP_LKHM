...
class DataLoader(object):
    ...
    def next_batch(self):
        ...
        alpha_feat = dataset["alpha_feat"][batch_index_inside_dataset * batch_size : (batch_index_inside_dataset + 1) * batch_size] # b x 1,0000 x 2
        ...
        if self.problem == "pdp" or self.problem == "cvrptw":
            ...
            self.batch_index += 1
            return (node_feat, edge_feat, alpha_feat, label1, label2, edge_index, inverse_edge_index)
        else:
            label = dataset["label"][batch_index_inside_dataset * batch_size : (batch_index_inside_dataset + 1) * batch_size] # b x 1000
            self.batch_index += 1
            return (node_feat, edge_feat, alpha_feat, label, edge_index, inverse_edge_index)
