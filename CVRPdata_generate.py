...
def generate_feat(dataset_name, instance, instance_name, max_nodes):
        ...
        for j in range(1,min((len(per_cand)+1)//2 , n_neighbours + 1)):
            ...
            if n_to not in edge_index[0][n_from]:
                ...
                if float(per_cand[2*j]) == 0:
                    ...
                    alpha = max(1,np.log10(float(per_cand[2*j]))+9)
    ...
    for idx in range(n_nodes):
        ...
        alpha_feat[0][idx][pos] = 10

    # save index and feature for candidateset node
    # edge_index = edge_index.reshape(1, -1, 20)
    # alpha_feat = alpha_feat.reshape(1, -1, 20)

    return edge_index, alpha_feat
...
def generate_dataset(n_samples, n_nodes, save_dir):
    ...
    edge_index, alpha_feat = list(zip(*feats))
    ...
    with Pool(20) as pool:
        results = list(tqdm.tqdm(pool.imap(method_wrapper, [("LKH", str(n_nodes), dataset[i], str(i), False, 10000) for i in range(len(dataset))]), total=len(dataset)))
